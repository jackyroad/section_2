...
static void TIM_Mode_Config(void)
{
  ...
  /* 使能通道1重载，写入TIMx_CCR1的值会先存入影子寄存器，在下一次更新事件（UEV）时自动生效，确保PWM周期完整性 */
  TIM_OC1PreloadConfig(ADVANCE_TIM, TIM_OCPreload_Enable);

  /*--------------------------------------------------------------*/
  /* 自动输出使能，断路、死区时间和锁定配置 */
  /* 在运行和空闲模式下当MOE=0 时，ENABLE:输出通道立即进入高阻态；
    DISABLE:输出通道保持由 OCx_IdleState 和 OCxN_IdleState 定义的静态电平 */
