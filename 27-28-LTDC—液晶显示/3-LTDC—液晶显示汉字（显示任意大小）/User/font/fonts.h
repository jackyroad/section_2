...
/* 如果另外制作使用比如32*32字模 需在 LCD_DispChar_CH 函数内需改取字节数据过程为对应字节个数：

  //取出4个字节的数据，在lcd上即是一个汉字的一行
		usTemp = ucBuffer [ ucPage * 4 ];
		usTemp = ( usTemp << 8 );
		usTemp |= ucBuffer [ ucPage * 4 + 1 ];
		usTemp = ( usTemp << 8 );
		usTemp |= ucBuffer [ ucPage * 4 + 2];
    usTemp = ( usTemp << 8 );
		usTemp |= ucBuffer [ ucPage * 4 + 3];
